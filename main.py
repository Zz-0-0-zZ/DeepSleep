...
from defense_models.deepsleep import DeepSleepDefense
...
if __name__ == '__main__':
    ...
    if defense == 'FGSMTrain':
        defense_model = FGSMTrain(model)
        robust_model = defense_model.advtrain(train_loader, eps=8/255, train_epoch=24)
    elif defense == 'PGDTrain':
        defense_model = PGDTrain(model)
        robust_model = defense_model.advtrain(train_loader, eps=8/255, train_epoch=24)
    elif defense == 'DefenseDist':
        defense_model = Distillation(model, train_loader=train_loader)
        robust_model = defense_model.distillation_train(temperature=10, train_epoch=24)
    elif defense == 'Sleep':
        defense_model = SleepDefense(model, train_loader=train_loader)
        robust_model = defense_model.sleep_defence(spike_rate=1/100, sleeplength=3)
        ...
        robust_model = defense_model.sleep_defence(sleep_depth=2, init_spike_rate=1/100, 
                                sub_sleeplength=1, rate_decrease_ratio=2, add_noise=False)
    else:
        robust_model = model
